...
//		}
		...
		/*
		 * if (down) { //Log.e(TAG, "touch:" + val); if (event.getAction() ==
		 * event.ACTION_DOWN || event.getAction() == event.ACTION_MOVE) { //
		 * calculate the new valimport com.csipsimple.R;ue based on touch if
		 * (orientation_horizontal) { val = (((ex - x) / w) * (max - min) +
		 * min); } else { val = (((h - (ey - y)) / h) * (max - min) + min); } //
		 * clamp the value val = Math.min(max, Math.max(min, val)); // send the
		 * result to Pd send("" + val); } else if (event.getAction() ==
		 * event.ACTION_UP) { down = false; } }
		 */
	}
